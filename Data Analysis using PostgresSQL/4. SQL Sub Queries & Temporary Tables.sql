...
-- average number of events for each channel. Average per day
...
-- list of orders happended at the first month in P&P history
SELECT * 
FROM orders
...
-- list of orders happended at the first day in P&P history
SELECT * 
FROM orders
...
-- name of the sales_rep in each region with the largest amount of sales
...
-- region with the largest (sum) of sales total_amt_usd and total order qty
...
-- largest standard paper orders accounts
...
-- Average number of events for each channel per day
WITH events as(
